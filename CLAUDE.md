...
5. `PreviewPanel` (non-activating `NSPanel`, `.borderless + .nonactivatingPanel`, level `.popUpMenu`) 가 `PreviewContentView`(SwiftUI 썸네일 그리드)를 호스팅. 키보드 네비(←→/Enter/Esc)는 panel 의 local event monitor 가 처리(`navState.selectedIndex`) — 단 **클릭으로 띄운 프리뷰만** key 라 키보드를 받는다(hover 프리뷰는 non-key, 아래 함정 참고). 콜백 5종(`onSelect`/`onClose`/`onSnap`/`onDismiss`/`onHoverWindow`)으로 AppDelegate 와 통신.
...
- **프리뷰 인터랙션 모델은 load-bearing (preview-ux 에서 도입)**: ① **hover 프리뷰는 키보드를 잡지 않는다** — `PreviewPanel.showPreview(grabsKeyboard:)` 는 click(=`AppDelegate.showPreviewForWindows(interactive:true)`)일 때만 `makeKey()`. hover 가 key 를 잡으면 사용자가 다른 앱에 타이핑 못 하는 freeze 가 재발한다. ② **좀비 프리뷰 방지 3중**: `PreviewPanel` 이 자기 `NSWindowDelegate` 로서 `windowDidResignKey`(클릭 프리뷰) + `AppDelegate` 의 `didActivateApplicationNotification` 옵저버(Cmd-Tab·앱 전환, hover·click 공통) + `isDismissing` 가드(`orderOut` 의 재진입 resignKey 차단). ③ **앱 전환은 패널 재사용**(`showPreview(reuse:)`) — dismiss+재페이드 없이 `rootView` 교체. 함정: `NSHostingView.fittingSize` 는 rootView 교체 직후 한 runloop 늦으므로 reframe 을 `DispatchQueue.main.async`(+`isVisible`/`dismissGeneration` 가드)로 미룬다. ④ **hover 게이팅**: 속도 게이트(`hoverVelocityThreshold` 1200pt/s — `pollMousePosition` 에서 `lastPollMouseLocation` 갱신 *전* 계산, 게이트는 `lastHoveredBundleID` 변경 *전* 배치) + 첫 hover 0.5s dwell + 브라우징 중 전환은 즉시.
